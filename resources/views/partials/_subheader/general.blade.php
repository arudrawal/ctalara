									...
									<div class="d-flex align-items-baseline flex-wrap mr-5">
										...
										<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
											<li class="breadcrumb-item text-muted">
												<a href="{{route('user.home')}}" class="text-muted">{{$selectedSponsor->code}}({{$selectedSponsor->id}})</a>
											</li>
											@if ($selectedStudy && $selectedStudy->id)
												<li class="breadcrumb-item text-muted">
													<a href="{{route('user.study.index')}}" class="text-muted">{{$selectedStudy->code}}</a>
												</li>
												@if ($selectedSite && $selectedSite->id)
													<li class="breadcrumb-item text-muted">
														<a href="{{route('user.site.index')}}" class="text-muted">{{$selectedSite->code}}</a>
													</li>
													@if ($selectedSubject && $selectedSubject->id)
														<li class="breadcrumb-item text-muted">
															<a href="{{route('user.subject.index')}}" class="text-muted">{{$selectedSubject->code}}</a>
														</li>
														@if ($selectedSubjectVisit && $selectedSubjectVisit->id)
															<li class="breadcrumb-item text-muted">
																<a href="{{route('user.subject.visit.index')}}" class="text-muted">{{$selectedSubjectVisit->order}}</a>
															</li>
															@if ($selectedSubjectVisitForm && $selectedSubjectVisitForm->id)
																<li class="breadcrumb-item text-muted">
																	<a href="{{route('user.subject.visit.form.index')}}" class="text-muted">{{$selectedSubjectVisitForm->form_id}}</a>
																</li>
															@endif
														@endif
													@endif
