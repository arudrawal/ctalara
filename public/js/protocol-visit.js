...
var KTDatatableProtocolVisit = function() {

	// initialize protocol visits table
	var initProtocolVisit = function(csrf_token) {
		const feSelect = document.getElementById("kt_datatable_search_protocol");
		const protocol_id = feSelect.options[feSelect.selectedIndex].value;
        var datatable = $('#kt_datatable_protocol_visit').KTDatatable({
            ...
            data: {
                ...
                source: {
                    read: {
						method: 'GET',
						url: '/api/protocol/visit/index/'+protocol_id,
            ...
            columns: [{
                ...
                title: 'Ref - Range',
				template: function(row) {
					return row['ref_visit_id'] +' - ' + row['range_start']+'/'+row['range_end']+ ' ' + row['display_range_unit']['name'];
				},
            }, {
                field: 'visit_forms',
                title: 'Visit Forms',
				template: function(row) {
					let formList = '';
					var formArray = row['visit_forms'];
					for (let idx=0; idx < formArray.length; idx++) {
						let displayForm = formArray[idx]['display_form'];
						formList = formList + formArray[idx]['order'] + " : " + displayForm['name'] + '<br/>';
					}
					return formList;
				},
            },{
	...
	var setProtocolFormValidator = function () {
		const protoForm = document.getElementById('protocol-visit-form');
