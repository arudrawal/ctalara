...
var KTDatatableProtocol = function() {

	// initialize protocol table
	var initProtocol = function(csrf_token, sponsor_id) {

        var datatable = $('#kt_datatable_protocols').KTDatatable({
            ...
            data: {
                ...
                source: {
                    read: {
						method: 'GET',
						url: '/api/protocol/index/'+sponsor_id,
        ...
        datatable.on('click', '[data-record-id]', function() {
			...
			if (this.dataset.action == 'edit') {
				...
				modalTitle.innerHTML = 'Edit Protocol';
				...
				if (rowJsonObj) {
					...
					$(modalBody).find('input[name=rev]').val(rowJsonObj.rev);
					$(modalBody).find('textarea[name=description]').val(rowJsonObj.description);
					$(modalBody).find('input[name=phase]').val(rowJsonObj.phase);
					$(modalBody).find('input[name=product]').val(rowJsonObj.product);
	...
	var setProtocolFormValidator = function () {
		...
		const fv = FormValidation.formValidation(protoForm, {
			fields: { 
				...
				rev: { // blank for Ajax errors
					validators: {
						blank: {},
					},
				},
				phase: { // blank for Ajax errors
					validators: {
						blank: {},
					},
				},
    ...
    return {
        ...
        init: function(csrf_token, sponsor_id) {
            initProtocol(csrf_token, sponsor_id);
        },
    };
}();
