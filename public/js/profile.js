...
var KTDatatableProfile = function() {

	// initialize security profile table
	var initProfile = function(csrf_token, sponsor_id) {
        ...
        var datatable = $('#kt_datatable_profile').KTDatatable({
            ...
            columns: [{
				...
				template: function(row) {
					let permList = '<table>';
					//permList += '<thead><th></th><th>View</th><th>Update</th><th>Delete</th></tr>';
					let permArray = row['permissions'];
					for (let idx=0; idx < permArray.length; idx++) {
						let displayModel = permArray[idx]['secure_model']['name'];
						let view = permArray[idx].view ? 'View': '';
						let update = permArray[idx].update ? 'Update' : '';
						let del = permArray[idx].delete ? 'Delete' : '';
						permList += `<tr><th>${displayModel}:</th>
									<td>${view}</td><td>${update}</td><td>${del}</td></tr>`;
					}
					permList = permList +'</table>';
					return permList;
				},
            },{
                field: 'Actions',
    ...
    return {
        // public functions
        init: function(csrf_token, sposnor_id) {
            initProfile(csrf_token, sposnor_id);
        },
    };
}();
